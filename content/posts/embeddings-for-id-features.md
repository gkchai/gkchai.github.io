...
ID features (`user_id`, `item_id`, `author_id`) are high-cardinality categorical signals. Instead of one-hot vectors, each ID can be mapped to a fixed embedding. The mapping can be done with a fixed vocab with a fixed set of out-of-vocab (OOV) embeddings, or through collision-prone hashing.
